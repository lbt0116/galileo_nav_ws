...
void VoxelOctoTree::init_plane(const std::vector<pointWithVar> &points, VoxelPlane *plane) {
    ...
    if (points.size() < 3) {
        std::cerr << "[init_plane] Not enough points: " << points.size() << std::endl;
        return;
    }
    ...
    if (!evalsReal.allFinite()) {
        std::cerr << "[init_plane] ERROR: Eigenvalues not finite! evalsReal = " << evalsReal.transpose() << std::endl;
        return;
    }
    ...
    Eigen::Index evalsMid = 3 - evalsMin - evalsMax;

    if (evalsMin == evalsMax) {
        std::cerr << "[init_plane] ERROR: Degenerate eigenvalues, min==max! evalsReal = " << evalsReal.transpose() << std::endl;
        return;
    }
    if (evalsMid < 0 || evalsMid >= 3 || evalsMid == evalsMin || evalsMid == evalsMax) {
        std::cerr << "[init_plane] ERROR: Illegal evalsMid: " << evalsMid << std::endl;
        std::cerr << "[init_plane] evalsReal: " << evalsReal.transpose() << std::endl;
        return;
    }
...
void VoxelMapManager::TransformLidar(const Eigen::Matrix3d rot, const Eigen::Vector3d t, const galileo_klio::CloudPtr &input_cloud,
...
void VoxelMapManager::BuildVoxelMap(const Eigen::Matrix3d rot, const Eigen::Matrix3d rot_cov,
    ...
    for (size_t i = 0; i < feats_down_world_->size(); i++) {
        ...
        M3D point_crossmat;
        point_crossmat = galileo_klio::SKEW_SYM_MATRIX(point_this);
...
void VoxelMapManager::pubVoxelMap() {
    ...
    cloud.reserve(pub_plane_list.size());
    for (const auto &pl : pub_plane_list) {
        if (!pl.is_plane_) continue;
        pcl::PointXYZI p;
        p.x = static_cast<float>(pl.center_(0));
        p.y = static_cast<float>(pl.center_(1));
        p.z = static_cast<float>(pl.center_(2));
        double trace = pl.plane_var_.block<3, 3>(0, 0).diagonal().sum();
        trace = std::min(trace, max_trace) * (1.0 / max_trace);
        trace = std::pow(trace, pow_num);
        p.intensity = static_cast<float>(1.0 - trace);
        cloud.push_back(p);
    }
...
bool VoxelMapManager::saveMapToPCD(const std::string &file_path) {
    ...
    for (const auto &pl : pub_plane_list) {
        ...
        p.y = static_cast<float>(pl.center_(1));
        p.z = static_cast<float>(pl.center_(2));
        p.intensity = static_cast<float>(pl.min_eigen_value_);
        cloud.push_back(p);
    }
    if (cloud.empty()) {
        RCLCPP_WARN(rclcpp::get_logger("voxel_map"), "saveMapToPCD: map is empty, nothing to save");
        return false;
    }
    int ret = pcl::io::savePCDFileBinary(file_path, cloud);
    if (ret == 0) {
        RCLCPP_INFO(rclcpp::get_logger("voxel_map"), "Map saved to %s with %zu points", file_path.c_str(), cloud.size());
        return true;
    } else {
        RCLCPP_ERROR(rclcpp::get_logger("voxel_map"), "Failed to save map to %s (code %d)", file_path.c_str(), ret);
        return false;
    }
}
...
void VoxelMapManager::pubSinglePlane(visualization_msgs::msg::MarkerArray &plane_pub, const std::string plane_ns,
    ...
    plane.header.frame_id = output_frame_id_;
    plane.header.stamp = rclcpp::Time();
    plane.ns = plane_ns;
    // 使用序列化的唯一 ID，避免同一数组中 (ns,id) 冲突
    plane.id = marker_id;
    plane.type = visualization_msgs::msg::Marker::CYLINDER;
    plane.action = visualization_msgs::msg::Marker::ADD;
    // 直接使用世界坐标（不再应用 outR_pub_ 旋转）
    Eigen::Vector3d c(single_plane.center_[0], single_plane.center_[1], single_plane.center_[2]);
    plane.pose.position.x = c.x();
    plane.pose.position.y = c.y();
    plane.pose.position.z = c.z();
    geometry_msgs::msg::Quaternion q;
    Eigen::Vector3d x_n = single_plane.x_normal_;
    Eigen::Vector3d y_n = single_plane.y_normal_;
    Eigen::Vector3d n   = single_plane.normal_;
    CalcVectQuation(x_n, y_n, n, q);
