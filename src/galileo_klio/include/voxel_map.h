...
class VoxelMapManager {
   public:
    ...
    galileo_klio::CloudPtr feats_undistort_;
    galileo_klio::CloudPtr feats_down_body_;
    galileo_klio::CloudPtr feats_down_world_;
    ...
    VoxelMapManager(VoxelMapConfig &config_setting) : config_setting_(config_setting) {
        current_frame_id_ = 0;
        feats_undistort_.reset(new galileo_klio::PointCloudType());
        feats_down_body_.reset(new galileo_klio::PointCloudType());
        feats_down_world_.reset(new galileo_klio::PointCloudType());
    };

    // 输出修正设置：由外部节点设置
    M3D outR_pub_ = M3D::Identity();
    std::string output_frame_id_ = "odom";

    // void StateEstimation(StatesGroup &state_propagat);
    void TransformLidar(const Eigen::Matrix3d rot, const Eigen::Vector3d t, const galileo_klio::CloudPtr &input_cloud,
    ...
    void pubVoxelMap();
    // 保存当前体素平面地图为PCD文件（以平面中心点作为点云）
    bool saveMapToPCD(const std::string &file_path);
