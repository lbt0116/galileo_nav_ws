...
private:
  ...
  void imuOnboardCallback(const sensor_msgs::msg::Imu::SharedPtr msg);
  // 旧的 RobotState 回调已移除
  ...
  // 基于机载IMU世界系（九轴）发布给地图节点的点云与里程计
  void publishMappingInputs(double stamp_sec);
  // 发布静态TF变换
  void publishStaticTF();
  // 保存地图服务回调
  void
  handleSaveMap(const std::shared_ptr<std_srvs::srv::Trigger::Request> request,
  ...
  rclcpp::Subscription<sensor_msgs::msg::Imu>::SharedPtr sub_imu_onboard_;
  ...
  rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr
      pub_pointcloud_to_map_;
  rclcpp::Publisher<nav_msgs::msg::Path>::SharedPtr pub_path_;
  rclcpp::Publisher<nav_msgs::msg::Path>::SharedPtr pub_path_to_map_;
  rclcpp::Publisher<nav_msgs::msg::Odometry>::SharedPtr pub_odom_world_;
  rclcpp::Publisher<nav_msgs::msg::Odometry>::SharedPtr pub_odom_to_map_;
  // 机身速度（IMU系）发布器
  rclcpp::Publisher<geometry_msgs::msg::TwistStamped>::SharedPtr
      pub_body_velocity_imu_;
  // 接触检测结果发布器（四腿）
  std::map<std::string,
  ...
  // 初始化阶段机载IMU姿态
  Eigen::Quaterniond imu_onboard_init_q_;
  bool has_imu_onboard_init_ = false;
  ...
  std::string imu_onboard_topic_;
  std::string kinematic_topic_;
  std::string cmd_vel_topic_;
  // 发布话题名（参数化）
  std::string topic_pointcloud_body_;
  std::string topic_pointcloud_world_;
  std::string topic_pointcloud_world_full_;
  std::string topic_path_;
  std::string topic_path_to_map_;
  std::string topic_odom_world_;
  std::string topic_odom_to_map_;
  std::string topic_pointcloud_to_map_;
  std::string topic_body_velocity_imu_;
  std::string topic_voxel_map_;
  std::string topic_contact_scores_prefix_;
  std::string topic_foot_position_prefix_;
  std::string topic_foot_velocity_prefix_;
  ...
  // 机载IMU最新消息（仅保存一个，不入队列）
  sensor_msgs::msg::Imu::SharedPtr imu_onboard_latest_;

  // 用户命令最新消息（仅保存一个，不入队列）
  geometry_msgs::msg::Twist::SharedPtr user_command_latest_;
