...
class Timer {
   ...
   private:
    ...
    void internalLogAllAverTime() {
        auto logger = rclcpp::get_logger("timer_utils");
        RCLCPP_INFO(logger, "===== Every Component Average Time =====");
        for (const auto& key : keys_) {
            RCLCPP_INFO(logger, "%s average time : %.3f ms", key.c_str(), average_time_map_[key].second);
        }
        RCLCPP_INFO(logger, "======================================");
    }
